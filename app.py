...
def create_app():
    ...
    _GRADES_POPUP = """<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="utf-8">
<title>Actualizar Calidades</title>
<style>
* { box-sizing: border-box; margin: 0; padding: 0 }
body {
  font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
...
h2 { font-size: 14px; font-weight: 700; color: #e0d4f7; }
p  { font-size: 11px; color: #a08cc0; line-height: 1.5; }
.file-row { display: flex; flex-direction: column; gap: 6px; }
.file-label { font-size: 11px; color: #a08cc0; text-transform: uppercase; letter-spacing: .07em; }
.file-input {
  background: #1a1a2e; border: 1px solid #3b2060; border-radius: 6px;
  padding: 9px 12px; font-size: 12px; color: #e0d4f7; cursor: pointer; width: 100%;
}
.file-input::-webkit-file-upload-button {
  background: #3b0764; color: #fff; border: none; padding: 4px 10px;
  border-radius: 4px; font-size: 11px; cursor: pointer; margin-right: 8px;
}
.btn-submit {
...
#status { font-size: 12px; min-height: 20px; color: #6fcf97; text-align: center; }
.err { color: #eb5757 !important; }
</style>
</head>
<body>
...
function upload() {
  ...
  fetch('/grades/upload', { method: 'POST', body: fd })
    ...
    @app.route('/grades/upload-popup')
    def grades_upload_popup():
        from flask import make_response
        resp = make_response(_GRADES_POPUP)
        resp.headers['Content-Type'] = 'text/html; charset=utf-8'
        return resp
    ...
    def grades_upload():
        ...
        for r in rows[header_idx + 1:]:
            ...
            if ticket_str in seen_tickets:
                continue
            seen_tickets.add(ticket_str)
            grades.append({
                'guia':          ticket_str,
                'grade':         grade,
    ...
    def list_bins():
        ...
        def _producers_match(a, b):
            return a in b or b in a or bool(_sig_words(a) & _sig_words(b))
        ...
        # Build producer_grade_map using: accent-normalized bidirectional substring,
        # with word-keyword fallback for cases like "DIEGO PACHECO" ↔ "AGRICOLA EL MONTE (PACHECO)"
        ...
        for _pw in _pw_names:
            ...
            for _dp_norm, _dg in _danos_prod_grade.items():
                if _producers_match(_dp_norm, _un):
    ...
    def debug_grade_counts():
        ...
        _GENERIC = {'AGRICOLA','AGRO','AGROCOM','LTDA','LIMITADA','INVERSIONES',
                    ...
                    'ALIMENTOS','FRUTAS','FRUTA'}
        def _sig_words(s):
            return {w for w in _re2.findall(r'[A-Z]{5,}', _norm(s)) if w not in _GENERIC}
        def _producers_match(a, b):
            return a in b or b in a or bool(_sig_words(a) & _sig_words(b))
        _gg_rows = db.session.query(GuiaGrade.productor, GuiaGrade.grade).filter(
            GuiaGrade.productor != None, GuiaGrade.productor != '').all()
    ...
    def list_pallets():
        ...
        if _pw_tarjas:
            for _pt, _tp in (db.session.query(HistoricoMovimiento.tarja, HistoricoMovimiento.tipoproceso)
                    ...
                    .filter(HistoricoMovimiento.tipoproceso.isnot(None))
                    .all()):
        ...
        total_saldo_kg = sum(v['kg'] for v in _tipo_st.values())
